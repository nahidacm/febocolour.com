  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl font-semibold text-foreground">Attributes ({items.length})</h1>
        <Link
        ...
        <Table>
          ...
          <tbody>
            {items.map((attr) => (
              <tr key={attr.id}>
                <Td>
                  <Link href={`/admin/attributes/${attr.id}`} className="font-medium hover:text-brand-700">
                    {attr.name}
                  </Link>
                </Td>
                <Td className="text-foreground/60">
                  {attr.inputType === "color_swatch" ? "Color Swatch" : "Select"}
                </Td>
                <Td className="text-foreground/60">{attr.values.map((v) => v.value).join(", ")}</Td>
