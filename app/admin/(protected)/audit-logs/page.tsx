...
export default async function AdminAuditLogsPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-foreground/60">Most recent {items.length} admin actions.</p>
      ...
      <div className="mt-6">
        <Table>
          ...
          <tbody>
            {items.map((log) => (
              <tr key={log.id}>
                <Td className="text-foreground/60">{new Date(log.createdAt).toLocaleString()}</Td>
                ...
                <Td className="max-w-xs truncate text-foreground/50">
        ...
        {items.length === 0 ? <p className="mt-4 text-sm text-foreground/60">No activity yet.</p> : null}
      </div>
    </div>
  );
}
